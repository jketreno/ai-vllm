...
def rpc_image(result: dict, name: str = "image") -> Image.Image:
    attachment = next(
        (item for item in result.get("attachments", []) if item.get("name") == name),
        None,
    )
    if not attachment:
        raise HTTPException(502, f"model worker did not return {name}")
    return Image.open(io.BytesIO(base64.b64decode(attachment["data_base64"]))).convert(
        "RGB"
    )


def optional_rpc_image(result: dict, name: str) -> Image.Image | None:
    if not any(item.get("name") == name for item in result.get("attachments", [])):
        return None
    return rpc_image(result, name)
...
def segment_response(source: Image.Image, result: dict, fields: set[str]) -> dict:
    ...
    for index, raw in enumerate(result.get("data", {}).get("segments", [])):
        ...
        if "mask" in fields:
            segment["mask"] = "data:image/png;base64," + base64.b64encode(
                mask_bytes
            ).decode("ascii")
        if "polygon" in fields:
            ...
            if contours:
                ...
                polygon = [
                    [round(float(point[0][0]), 1), round(float(point[0][1]), 1)]
                    for point in approx
                ]
    ...
    return {
        ...
        + base64.b64encode(png_bytes(overlay)).decode("ascii"),
        "width": source.width,
