...
corpus_tokens_total = Gauge(
    "clare2_corpus_tokens_total", "SFT token count", ["project"]
)
...
training_loss_final = Gauge(
    "clare2_training_loss_final", "Last final loss", ["project"]
)
...
distillation_runs = Counter(
    "clare2_distillation_runs_total", "Distillation runs", ["outcome"]
)
distillation_sessions_pending = Gauge(
    "clare2_distillation_sessions_pending",
    "Session files pending distillation",
    ["project"],
)
distillation_sessions_last = Gauge(
    "clare2_distillation_sessions_last",
    "Session files processed by the last distillation run",
    ["project"],
)
distillation_sessions = Counter(
    "clare2_distillation_sessions_total",
    "Distillation session outcomes",
    ["project", "outcome"],
)
...
distillation_patterns_raw = Counter(
    "clare2_distillation_patterns_raw_total",
    "Raw patterns returned by distillation",
    ["project"],
)
...
distillation_patterns_extracted = Counter(
    "clare2_distillation_patterns_extracted",
    "Distilled patterns",
    ["project", "category"],
)
...
summary_records_input = Gauge(
    "clare2_summary_records_input",
    "Input records for the last summary run",
    ["project", "level"],
)
summary_records_output = Gauge(
    "clare2_summary_records_output",
    "Output records from the last summary run",
    ["project", "level"],
)
summary_last_run_timestamp = Gauge(
    "clare2_summary_last_run_timestamp_seconds",
    "Last summary run timestamp",
    ["project", "level"],
)
...
routing_decisions = Counter(
    "clare2_routing_decisions_total", "Routing decisions", ["rule"]
)
...
adapter_operation_latency = Histogram(
    "clare2_adapter_operation_duration_seconds",
...
image_lease_duration = Histogram(
    "clare2_image_edit_lease_duration_seconds",
    "Image-edit resource lease duration",
...
notification_sent = Counter(
    "clare2_notification_sent_total",
    "Run notification emails sent",
    ["outcome", "status"],
)
