...
def _recent_episode_paths(project: str, days: int) -> list[pathlib.Path]:
    ...
    if not project_dir.exists():
        return []
    dated_paths = [
        (_episode_date(path), path) for path in project_dir.glob("*/*/*.jsonl")
    ]
    return [path for _date, path in sorted(dated_paths, reverse=True)[:days]]


def _load_recent_episodes(
    project: str, days: int = RECENT_EPISODE_WINDOW_DAYS
) -> list[dict]:
    episodes: list[dict] = []
...
_PROMPT_VARIANTS: dict[str, list[str]] = {
    "antipattern": [
        "{context}\nYou are modifying this repository and encounter the "
        "following anti-pattern.\nAnti-pattern: {description}\n\nRespond "
        "with the concrete behavior you should apply instead.",
        "{context}\nA teammate proposes the change below, which repeats a "
        "known anti-pattern here.\nAnti-pattern: {description}\n\nExplain "
        "what you would do instead and why.",
        "{context}\nCode review flagged the following recurring "
        "anti-pattern in this codebase.\nAnti-pattern: {description}\n\n"
        "Describe the correction you would request.",
    ],
    "style": [
        "{context}\nYou are preparing a code or documentation change for "
        "this repository.\nStyle convention: {description}\n\nRespond "
        "with how you should apply this convention.",
        "{context}\nA new contributor asks how code in this repository "
        "should be formatted.\nStyle convention: {description}\n\n"
        "Explain the convention and when it applies.",
        "{context}\nYou are reviewing a diff that may not follow this "
        "repository's conventions.\nStyle convention: {description}\n\n"
        "State what you would change and why.",
    ],
    "architecture": [
        "{context}\nYou are designing or changing a component in this "
        "repository.\nArchitectural pattern: {description}\n\nRespond "
        "with the implementation approach that respects this architecture.",
        "{context}\nA proposed change risks cutting across this "
        "repository's established boundaries.\nArchitectural pattern: "
        "{description}\n\nExplain how you would keep the change "
        "consistent with it.",
        "{context}\nSomeone asks where new logic like this belongs in "
        "the codebase.\nArchitectural pattern: {description}\n\nDescribe "
        "where and how you would place it.",
    ],
    "domain": [
        "{context}\nYou are answering or implementing a change for this "
        "repository.\nDomain rule: {description}\n\nRespond with the "
        "correct project-specific handling.",
        "{context}\nA question comes up about how this project handles a "
        "specific situation.\nDomain rule: {description}\n\nExplain the "
        "project-specific answer.",
        "{context}\nYou need project-specific context before implementing "
        "a related change.\nDomain rule: {description}\n\nState the rule "
        "and how it shapes your implementation.",
    ],
}
...
_COMPLETION_VARIANTS: dict[str, list[str]] = {
    "antipattern": [
        ...
        "Instead, verify the behavior through the repository's "
        "authoritative path, preserve the existing project constraints, "
        "and explain the safer implementation choice.{reference}",
        "This is a known anti-pattern here: {description}\n"
        "I'd request the safer alternative instead — one that keeps the "
        "repository's existing constraints intact and is verifiable "
        "through its authoritative path.{reference}",
        "That repeats {description}\n"
        "The fix is to route through the repository's authoritative path "
        "rather than the shortcut, keeping the project's existing "
        "constraints intact.{reference}",
    ],
    "style": [
        "Apply the style convention consistently: {description}\n"
        "Use the convention where it affects prose or code users will "
        "see, while preserving literal identifiers, paths, environment "
        "variables, and protocol values.{reference}",
        "This repository's convention is: {description}\n"
        "I'd apply it anywhere it affects visible prose or code, without "
        "touching literal identifiers, paths, environment variables, or "
        "protocol values.{reference}",
        "Following this project's style: {description}\n"
        "That convention governs visible code and prose here, but "
        "literal values like paths, env vars, and protocol strings stay "
        "untouched.{reference}",
    ],
    "architecture": [
        "Follow the established architecture: {description}\n"
        "Keep ownership boundaries explicit, derive generated artifacts "
        "from their sources, and add focused verification for the "
        "invariant being protected.{reference}",
        "The established pattern here is: {description}\n"
        "I'd keep ownership boundaries explicit, derive any generated "
        "artifacts from their source, and add targeted verification for "
        "the invariant it protects.{reference}",
        "This codebase's architecture calls for: {description}\n"
        "New logic should respect that boundary, source generated output "
        "from its origin, and be checked against the specific invariant "
        "it exists to protect.{reference}",
    ],
    "domain": [
        "Use the project-specific rule directly: {description}\n"
        "Prefer the repository's configured source of truth over "
        "assumptions, and make the resulting behavior observable through "
        "the normal project workflow.{reference}",
        "In this project: {description}\n"
        "I'd rely on the repository's configured source of truth rather "
        "than assuming, and surface the result through the normal "
        "project workflow.{reference}",
        "The project-specific answer is: {description}\n"
        "That comes from the repository's configured source of truth, "
        "not assumption, and should be observable through the usual "
        "workflow.{reference}",
    ],
}
...
def _variant_index(pattern: dict, variants: list[str]) -> int:
    key = (
        f"{pattern.get('session_id', '')}|{pattern.get('category', '')}"
...
def _pattern_to_sft_pair(pattern: dict) -> dict | None:
    ...
    completion_variants = _COMPLETION_VARIANTS.get(
        category, _COMPLETION_VARIANTS["domain"]
    )
    variant_index = _variant_index(pattern, prompt_variants)
    prompt = prompt_variants[variant_index].format(
...
def _assemble_project(project: str) -> dict:
    ...
    if not sft_pairs:
        log.warning(
            "No SFT pairs generated for project %s — corpus may be empty", project
        )
...
def assemble() -> dict:
    """Generate per-project training/current.jsonl from active themes + episodes."""
    ...
    for project in projects:
        ...
        metrics.corpus_tokens_total.labels(project=project).set(
...
def _update_manifest(
    project: str, sft_pairs: list[dict], estimated_tokens: int
) -> None:
