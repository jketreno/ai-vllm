...
def _load_active_themes() -> list[dict]:
    themes: list[dict] = []
    theme_dir = CORPUS_ROOT / "themes" / "active"
...
def _load_recent_episodes(days: int = RECENT_EPISODE_WINDOW_DAYS) -> list[dict]:
    episodes: list[dict] = []
...
def assemble() -> dict:
    """Generate corpus/training/current.jsonl from active themes + recent episodes."""
    themes = _load_active_themes()
    ...
    if not sft_pairs:
        log.warning("No SFT pairs generated — corpus may be empty")

    training_dir = CORPUS_ROOT / "training"
    ...
    metrics.corpus_tokens_total.set(estimated_tokens)

    _update_manifest(sft_pairs, estimated_tokens)

    log.info("Corpus assembled: %d SFT pairs, ~%d tokens", len(sft_pairs), estimated_tokens)
    return {"sft_pairs": len(sft_pairs), "estimated_tokens": estimated_tokens}


def _update_manifest(sft_pairs: list[dict], estimated_tokens: int) -> None:
    manifest_path = CORPUS_ROOT / "training" / "manifest.json"
    try:
